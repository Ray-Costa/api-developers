...
import { DevelopersInfoRequiredKeys, DevelopersRequiredKeys } from '../interfaces/interface';
 ...
 export const developersExists = async (request:Request, response:Response, next:NextFunction): Promise<Response | void> => {
   ...
   const queryString:string =`
    SELECT COUNT(*) FROM developers WHERE id = $1;
   `
...
export const validateBodyInfosMiddleware = async (request:Request, response: Response, next:NextFunction): Promise<Response | void> =>{
   ...
   const keys: Array<string> = Object.keys(request.body);
   const requiredKeys: Array<DevelopersInfoRequiredKeys> = ["developerSince","preferredOS"];

   const validatedInfoKeys: boolean = requiredKeys.every(
     (key:string) => keys.includes(key) && keys.length === requiredKeys.length || keys.length > requiredKeys.length
   );
   if(!validatedInfoKeys){
     return response.status(400).json({message: `At least one of those keys must be send. Keys:${requiredKeys}`});
   }
   if(!requiredKeys){
     return response.status(400).json({message: `At least one of those keys must be send. Keys:${requiredKeys}`});
